...
<article id="post-<?php the_ID(); ?>" <?php post_class('type-entry-outer'); ?>>
	...
	<div class="type-entry">
		...
		<h3 class="type-entry-title">
			...
			<?php if ( has_excerpt() ) : ?>
				<span class="type-entry-excerpt"><?php if ( has_category('quotes')) : ?> <i class="fas fa-briefcase"></i><?php elseif ( has_category('books-documents')) : ?> <i class="fas fa-book"></i><?php else: ?><i class="fas fa-external-link-alt"></i><?php endif; ?><?php echo get_the_excerpt(); ?></span>
			<?php endif; ?>
		</h3>
		...
		<div class="type-entry-bottom mobile-bottom group">
			...
			<div class="type-entry-date"><a href="<?php echo get_month_link( $archive_year, $archive_month ); ?>" title="Archive for <?php the_time('F Y'); ?>"><i class="fas fa-clock"></i><?php the_time('M d'); ?></a></div>
						
			<?php if ( has_category() ): ?>
				<div class="type-entry-category group"><?php exclude_post_categories('11'); ?></div>
			<?php endif; ?>
			
			<?php if ( rwmb_get_value( 'extra' ) ): ?>
				<button class="type-entry-extra-btn" title="Additional Information"><div class="type-entry-extra-btn-curve"></div><div class="type-entry-extra-btn-bg"></div><i class="fas fa-info-circle"></i></button>
			<?php endif; ?>
			
			<?php if ( comments_open() ): ?>
				<?php $number = get_comments_number( $post->ID ); if ( $number > 0 ) { ?>
					<a class="type-entry-comments" href="<?php comments_link(); ?>"><i class="fas fa-comment"></i><span><?php comments_number( '0', '1', '%' ); ?></span></a>
				<?php } ?>
			<?php endif; ?>
